		...
		if (resource == nullptr) {
			...
			if (ImGui::Checkbox("Set time parameters", &setTimeParam)) {
				if (setTimeParam) {
					...
					fxParamManager->paramInterpolators.godrayInterpolator->AddJob(timeJobId, &resource->GetData()->timeItems[timeItemId].godray, 0xFFFFFFFF, 20001, 0.0f, -1);
					...
					fxParamManager->paramInterpolators.atmosphereInterpolator->AddJob(timeJobId, &atmosphereParam, 0x7FDD2C, 20001, 0.0f, -1);
			...
			if (ImGui::Checkbox("Overlay weather parameters", &setWeatherParam)) {
				if (setWeatherParam) {
					...
					fxParamManager->mutex.Lock();
					fxParamManager->paramInterpolators.bloomInterpolator->AddJob(weatherJobId, &resource->GetData()->weatherItems[weatherItemId].bloom, 0xFFFFFFFF, 20002, 0.0f, -1);
					fxParamManager->paramInterpolators.cameraControlInterpolator->AddJob(weatherJobId, weatherUseEventCameraControl ? &resource->GetData()->weatherItems[weatherItemId].cameraControlEvent : &resource->GetData()->weatherItems[weatherItemId].cameraControl, 0xFFFFFFFF, 20002, 0.0f, -1);
					fxParamManager->paramInterpolators.tonemapInterpolator->AddJob(weatherJobId, &resource->GetData()->weatherItems[weatherItemId].tonemap, 0xFFFFFFFF, 20002, 0.0f, -1);
					...
					fxParamManager->paramInterpolators.effectInterpolator->AddJob(weatherJobId, &resource->GetData()->weatherItems[weatherItemId].effect, 0xFFFFFFFF, 20002, 0.0f, -1);
					...
					//fxParamManager->paramInterpolators.weatherInterpolator->AddJob(weatherJobId, &resource->GetData()->weatherItems[weatherItemId].weather, 0xFFFFFFFF, 20002, 0.0f, -1);
					...
					fxParamManager->paramInterpolators.atmosphereInterpolator->AddJob(weatherJobId, &atmosphereParam, 0x7FFD2C, 20002, 0.0f, -1);
					fxParamManager->mutex.Unlock();
				}
				else {
					fxParamManager->mutex.Lock();
