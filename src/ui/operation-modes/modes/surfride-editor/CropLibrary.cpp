				...
				if (currentPath.size() == 0) {
					for (size_t i = 0; i < context.project->textureListCount; i++) {
						...
						if (ImBrowser::Item(&textureList, textureList.name, false, nullptr, nullptr, nullptr, 0, browserIcon, {}) && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
							currentPath.push_back(texture_editor::TextureListRef{ context.gocSprite->GetProject(), i });
						ImGui::PopID();
					}
				}
				else {
					...
					case texture_editor::TextureElement::Type::TEXTURELIST: {
						...
						for (size_t i = 0; i < textureList.textureCount; i++) {
							...
							auto* name = texture.filename;
#else
							auto* name = texture.name;
#endif

							ImGui::PushID(&texture);
							if (ImBrowser::Item(&texture, name, false, nullptr, nullptr, nullptr, 0, browserIcon, browserThumbnail) && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
								currentPath.push_back(texture_editor::TextureRef{ context.gocSprite->GetProject(), top.textureList.textureListIndex, i });
							ImGui::PopID();
						}
						break;
					}
					case texture_editor::TextureElement::Type::TEXTURE: {
						...
						for (size_t i = 0; i < texture.cropCount; i++) {
							...
							auto icon = GetIconAsImage(IconId::RESOURCE_TEXTURE);
							ImBrowser::Image browserIcon{ .texId = icon.texId, .uv1 = icon.uv1, .uv2 = icon.uv2, .size = icon.size };
							...
							if (ImGui::IsItemActive() && ImGui::BeginDragDropSource()) {
								texture_editor::CropRef cropRef{ context.gocSprite->GetProject(), top.texture.textureListIndex, top.texture.textureIndex, i };
